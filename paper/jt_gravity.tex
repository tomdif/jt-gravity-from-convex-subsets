...
\log w_i)^2$, with $R^2 = 0.85$ at $\beta = 1$ rising to $0.90$ at
$\beta = 3$. At $n = 30$ (50 profiles), the topological
...
$\beta$ ($R^2 > 0.99$) at both system sizes, confirming the
decomposition into
...
The density of states is Hagedorn-type: a Gaussian envelope
in log-space ($R^2 = 0.996$) fits better than the JT
$\sinh\sqrt{E}$ prediction ($R^2 = 0.991$), with a hard UV cutoff
at $S_{\mathrm{BD,max}} = \min(d,m)$ --- the Dilworth width of the
poset, fixed by the spacetime dimension and independent of the
lattice spacing.
...
arbitrary subsets. This is a discrete path integral over geometries.
...
question is whether the partition function~(\ref{eq:Z}) reproduces
this structure.

We find that it does. On 42 variable-width grids at fixed size
$n = 15$, the effective action extracted from $Z(\beta)$ decomposes
into a topological (Einstein-Hilbert) term and a conformal-mode
kinetic (Schwarzian) term, with $R^2 = 0.85$ and both coefficients
...
\begin{equation}
\label{eq:BD}
S_{\mathrm{BD}}[S] = |S| - 2 \cdot |\{(x,y) \in S \times S : x \lessdot y\}|,
\end{equation}
where $x \lessdot y$ denotes a link (covering relation): $x < y$
with no $z$ satisfying $x < z < y$. On a poset embedded in a
Lorentzian manifold, the continuum limit of $S_{\mathrm{BD}}$
converges to the Einstein-Hilbert action $\int R \sqrt{g}\,
...
configuration):
\begin{equation}
S_{\mathrm{BD}}([m]^d) = -m^d + 2m.
\end{equation}
This is the unique minimum. For $\beta > 0$, the partition
function~(\ref{eq:Z}) is dominated by this configuration: flat
spacetime is the saddle point.

On $[4]^2$: the full grid has $S_{\mathrm{BD}} = -8$ (unique
minimum among 1{,}146 convex subsets). The anti-diagonal antichain
$\{(0,3),(1,2),(2,1),(3,0)\}$ has $S_{\mathrm{BD}} = +4$ (unique
maximum). The density of states peaks at $S_{\mathrm{BD}} \approx 1$;
...
we generated 42 variable-width grid profiles with fixed element count
$n = 15$ and discrete curvatures ranging from $R = -10$ to
$R = +10$. Profiles were constructed by systematic enumeration of
width sequences $(w_0, \ldots, w_{T-1})$ with $\sum w_t = 15$,
$T$ ranging from 3 to 8, and $w_t \geq 1$.
...
action is $S_{\mathrm{BD}}(\text{full}) = n - 2[(n-T) + \sum
\min(w_t, w_{t+1})]$, which depends primarily on~$R$.

Regressing each term separately against geometric invariants on our
42 profiles reveals a clean orthogonal decomposition:
...
\begin{ruledtabular}
\begin{tabular}{lccc}
Component & Best predictor & $R^2$ & Cross-$R^2$ \\
\midrule
Action $(-\beta \cdot S_{\mathrm{BD, full}})$ & $R$ & 0.19 & --- \\
Entropy (remainder) & $\mathrm{LogSch}$ & 0.69 & 0.10 vs $R$ \\
\midrule
Combined $\log Z$ & $R + \mathrm{LogSch}$ & 0.85 & --- \\
\end{tabular}
\end{ruledtabular}
\end{table}

The action term depends on the discrete curvature $R$ ($R^2 = 0.19$)
and is nearly independent of the log-Schwarzian. The entropy term
depends on the log-Schwarzian ($R^2 = 0.69$) and is nearly
orthogonal to $R$ ($R^2 = 0.10$ against~$R$). The two predictors
explain \emph{different} components of $\log Z$ with almost no
overlap, and their combination gives $R^2 = 0.85$.

This is not a phenomenological fit: it is a decomposition into
physically distinct contributions. The curvature $R$ controls the
...
profile not captured by the two-parameter model. Adding boundary
terms and higher-order invariants at $n = 15$ improved $R^2$ from
$0.85$ to $0.89$ (Table~\ref{tab:regression15}); a similar analysis
at $n = 30$ is expected to close part of the gap.

\begin{table}[h]
\caption{Extended regression at $n = 15$ (42 profiles).}
...
Hagedorn-type system --- exponential growth capped by a maximum
energy --- rather than pure JT gravity.

The UV cutoff has a combinatorial origin: the maximum BD action on
any convex subset of $[m]^d$ is $S_{\mathrm{BD, max}} = \min(d, m)$,
the Dilworth width of the poset. For $m \geq d$, the cutoff is
$S_{\mathrm{BD, max}} = d$, fixed by the spacetime dimension
and independent of the lattice spacing. This is a hard boundary in
the spectrum that persists in the continuum limit --- a UV
completion built into the combinatorial structure.
...
path integral. Its state space is the set of causally consistent
subregions, its action is the Benincasa-Dowker functional, and the
sum is finite. No continuum limit, regularization, or gauge-fixing
is required.

In two dimensions, the effective action extracted from $Z(\beta)$
reproduces the structure of Jackiw-Teitelboim gravity. At $n = 15$,
...
computable realization of the sum over geometries in quantum gravity.
It makes no reference to a background metric, a continuum manifold,
or quantum field theory. The geometries summed over are the causally
